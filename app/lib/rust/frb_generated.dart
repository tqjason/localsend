...
abstract class RustLibApi extends BaseApi {
  ...
  Future<ResultWithPublicKeyPrepareUploadResponseDto>
  crateApiHttpRsHttpClientPrepareUpload({
    required RsHttpClient that,
    required ProtocolType protocol,
    required String ip,
    required int port,
...
class RustLibApiImpl extends RustLibApiImplPlatform implements RustLibApi {
  ...
  crateApiHttpRsHttpClientPrepareUpload({
    ...
    return handler.executeNormal(
      NormalTask(
        ...
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_result_with_public_key_prepare_upload_response_dto,
          decodeErrorData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerClientError,
        ),
        constMeta: kCrateApiHttpRsHttpClientPrepareUploadConstMeta,
        argValues: [that, protocol, ip, port, payload, pin],
        apiImpl: this,
      ),
    );
  }
  ...
  TaskConstMeta get kCrateApiHttpRsHttpClientPrepareUploadConstMeta =>
      const TaskConstMeta(
        ...
        argNames: ['that', 'protocol', 'ip', 'port', 'payload', 'pin'],
      );
  ...
  Future<void> crateApiHttpRsHttpClientUpload({
    ...
    return handler.executeNormal(
      NormalTask(
        ...
        argValues: [that, protocol, ip, port, sessionId, fileId, token, binary],
        apiImpl: this,
      ),
    );
  }
  ...
  @protected
  ResultWithPublicKeyPrepareUploadResponseDto
  dco_decode_result_with_public_key_prepare_upload_response_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return ResultWithPublicKeyPrepareUploadResponseDto(
      publicKey: dco_decode_opt_String(arr[0]),
      body: dco_decode_prepare_upload_response_dto(arr[1]),
    );
  }
  ...
  sse_decode_result_with_public_key_prepare_upload_response_dto(
    ...
    var var_body = sse_decode_prepare_upload_response_dto(deserializer);
    return ResultWithPublicKeyPrepareUploadResponseDto(
  ...
  @protected
  void sse_encode_result_with_public_key_prepare_upload_response_dto(
    ResultWithPublicKeyPrepareUploadResponseDto self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_String(self.publicKey, serializer);
    sse_encode_prepare_upload_response_dto(self.body, serializer);
  }
